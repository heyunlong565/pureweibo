 ...
 * <p>HTML模板字符解析依赖两个数据：
    ...
    htmls字符串是原始的数据，通过{@link #parse}方法
   </p>
 */
 ...
 */    
    ...
    /*
    insertText : function(inputor, text){
        if (document.selection) {
           ...
           inputor.value = 
                inputor.value.substring(0, start) + 
                text + inputor.value.substring(end, inputor.value.length);
        
           inputor.selectionStart = inputor.selectionEnd = start + text.length;
    ...
    */
    ...
    delSelection : function(elem){

        end += start;
        var v = elem.value;
        //elem.value = stringReplace
    },
 ...
 */
        createView : function(){
            var v = this.view;
            if(typeof v === 'string'){
                v = this.view = T.forNode(T[v], this, TRUE);
