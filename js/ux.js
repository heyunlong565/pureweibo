...
var 
	...
	function cssShow(ele) {
		return $(ele).removeClass(hideCls);
	}

	//隐藏
	function cssHide(ele) {
		return $(ele).addClass(hideCls);
	}
...
ui.CommentArea = X.reg('CmtArea', Util.create(Base, {
    ...
    createCmtUI : function(cmt){
        var wbUid = this.getFromUser(),
			UID = getUid();
        cmt.sinaV = cmt.sina_v;
        cmt.usrUrl= Req.mkUrl( 'ta', '', 'id='+cmt.uid );
...
ui.PostBase = {
    ...
    post : function(){
        ...
        this.sending = TRUE;
        // add disabledcs
        Req.post(text, Util.getBind(this, 'onSendLoad'), this.uploadPic);
    },
...
ui.updateCount = function() {
	...
	Req.counts(ids.join(','), function(e) {
		if (e.isOk())
		{
			...
			/*
			$.each(e.getData(), function(i, c) {
				var $wb = $('div.feed-list>ul>li[rel=w:'+i+']');
				//转发数
				$wb.find('div.hot-total>strong').text(c[1]);
				c[1] && $wb.find('a[rel=l:fw],a[rel=e:fw]').text('转发('+c[1]+')');
				c[0] && $wb.find('a[rel=l:cm],a[rel=e:cm]').text('评论('+c[0]+')');
