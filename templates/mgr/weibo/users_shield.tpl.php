...
<body>
<div class="main-wrap">
	<div class="path"><span class="path-icon"></span>当前位置：运营管理<span> &gt; </span>微博管理<span> &gt; </span>用户屏蔽</div>
    <div class="set-wrap">
        	...
        	<div class="user-list-box1">
            	...
            	<div class="serch-user">
            		<form action="<?php echo URL('mgr/weibo/disableUser.search');?>" method="post">
                		...
                		<span><input name="keyword" class="input-box box-address-width" type="text" value="<?php if (V('r:keyword', false)) echo V('r:keyword');?>" /></span>
                ...
                <ul class="serch-results">
                	...
                	<li class="result-line<?php if ($c <= 1) {?>-no<?php }?>">
                        ...
                        <div class="results-r">
							<?php if ($row['disabled']) {?>
                        	<span>已屏蔽</span>
                            <a href="javascript:confirmDel('<?php echo URL('mgr/weibo/disableUser.resume','uid='.$row['id'], 'admin.php');?>', '确认要解除封禁吗?')" class="unban">解除屏蔽</a>
							<?php } else {?>
							<a href="javascript:confirmDel('<?php echo URL('mgr/weibo/disableUser.disable','uid='.$row['id'] . '&nick=' , 'admin.php');?>' + encodeURIComponent('<?php echo $row['screen_name']?>'),'确认要屏蔽该用户吗?')" class="confirmation">确认屏蔽</a>
							<?php }?>
                        </div>
                    </li>
